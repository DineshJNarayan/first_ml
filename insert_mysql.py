...
##         mydb = mysql.connector.connect(host="127.0.0.1", user="root", password="Vmcp2020Gain", database="MachineLearning")
##         mycursor = mydb.cursor()
##         sql = "INSERT INTO diabetes_data (pregnancies, glucose, bloodpressure, skinthickness, insulin, bmi, diabetespedigree, age, outcome) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)" 
##         val = (str(pregnancies), str(glucose), str(bloodpressure), str(skinthickness), str(insulin), str(bmi), str(diabetespedigree), str(age), str(outcome)) 
##         mycursor.execute(sql, val)
##         mydb.commit()
